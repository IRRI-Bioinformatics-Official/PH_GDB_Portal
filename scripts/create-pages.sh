...
if (empty(\$jb)) {
  \$n2 = \Drupal\node\Entity\Node::create([
    ...
    'path' => ['alias' => '/data/jbrowse'],
  ]);
  \$n2->save();
  echo 'JBrowse created: node ' . \$n2->id() . PHP_EOL;
} else {
  echo 'JBrowse already exists.' . PHP_EOL;
}

// ── JBrowse2 ──────────────────────────────────────────────────
\$jb2 = \Drupal::entityTypeManager()->getStorage('node')->loadByProperties(['title' => 'JBrowse2 Genome Browser']);
if (empty(\$jb2)) {
  \$n3 = \Drupal\node\Entity\Node::create([
    'type' => 'page',
    ...
    'path' => ['alias' => '/data/jbrowse2'],
  ]);
  \$n3->save();
  echo 'JBrowse2 created: node ' . \$n3->id() . PHP_EOL;
} else {
  echo 'JBrowse2 already exists.' . PHP_EOL;
}
"
